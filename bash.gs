...
//BUG if objects map is blank then displaying map causes CTD
...
bat.rid="CO02JVVK0000001662929292230656"
bat.logged_in=0
...
login=function
    ...
    password=user_input("Password:*********".center,true)
    if password==bio.sys_pwd then
        ...
        print char(10)
        user_input("<b><align=center>CRITICAL SYSTEM ERROR: Password Invalid".color("#ff0000")+char(10)+"The Password Entered Is Invalid.  System functionality will be <b>".color("#FFFFFF")+"DISABLED </b>".color("#AA0000")+"until a valid password is entered.".color("#FFFFFF"),0,1);return 
    end if
    if bio.mode=="sp" then
        cor.objects("add",bio.master_shell)
        bat.cur_obj==bio.master_shell
    ...
    et=function
            ...
            if bio.decoy_servers.len>0 then
                    ...
                    if typeof(bio.fake_server)=="shell" then
                        ...
                        if typeof(bio.fake_meta)!=null and bio.debug==1 then;print typeof(bio.fake_meta)+" Bot Online";user_input("Press Enter To Continue",0,1);end if
                    end if
            else
...
bat.run=function()
    ...
    while true
        ...
        if bio.params.len==0 then 
            params = user_input(bat.usr + "@" + promptCurrentFolder + " ",bio.demo, false, true)
            params=params.split(" ")
        else
            params=bio.params
        end if
